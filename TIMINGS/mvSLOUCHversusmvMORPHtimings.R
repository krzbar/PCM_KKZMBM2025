...
v_doneN<-c(5,10,30,50,100,200,500,1000) ##2000,5000,10000,20000,50000)
vN<-c(1500,2000)
numreps<-30
...
for (n in vN){
    ...
    max_y<-max(c(max_y,mediantime+interquant_time[2,]),na.rm=TRUE)
    i<-i+1
}
max_y<-max_y+0.05
...
v_ylim<-c(0.01,max_y)
##(max(c(l_timings$mMedians[numN,1]+l_timings$lInterQuantiles[[numN]][2,1],l_timings$mMedians[numN,2]+l_timings$lInterQuantiles[[numN]][2,2],l_timings$mMedians[numN,3]+l_timings$lInterQuantiles[[numN]][2,3]),na.rm=TRUE)+0.05))
c_ylab<-"time[s]"
if (c_plotlogscale){v_ylim<-log(v_ylim);c_ylab<-"log(time[s])"}
plot(NA,xlim=c(0,max(vN)+1),ylim=v_ylim,main="",xlab="n",ylab=c_ylab,cex.axis=1.25,font.axis=2,font.lab=2,cex.lab=1.5)
	...
	for (n in vN){
	    ...
	    segments(n,interquant_time[1], n, interquant_time[2], col=vcolours[j],lwd=8)
	    segments(n-len_bar,interquant_time[1], n+len_bar, interquant_time[1], col=vcolours[j],lwd=8)
	    segments(n-len_bar,interquant_time[2], n+len_bar, interquant_time[2], col=vcolours[j],lwd=8)
	    i<-i+1
    ...
    legend("topright",legend=c("mvSLOUCH 2.7.6","mvMORPH 1.2.1 pic","mvMORPH 1.2.1 rpf"),col=vcolours,pch=19,bty="n",cex=1.25,text.font=2)
dev.off()
sink()
