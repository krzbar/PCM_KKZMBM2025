...
# fopt_mvs <- function(v, ntips, ids){
#   # convert v to ParamsInModel and M.error
#   pars <- list(Sxx = matrix(c(v[1], v[2], 0, v[3]), nrow = 2),
#                vX0 = matrix(v[4:5], ncol = 1))
# 
#   # create SPD matrix
#   L <- matrix(c(v[6], v[7], 0, v[8]), nrow = 2)
#   M <- L %*% t(L)
# 
#   Merror <- sapply(1:ntips, function(x){matrix(0,2,2)}, simplify=FALSE)
# 
...
#   }
# 
#   res <- SummarizeBM(phyltree = tree, mData = Xsim, modelParams = pars,
#                      M.error = Merror)$t_1$LogLik
# 
#   if (is.infinite(res)){
#     return(-1e5)
#   }else{
...
#     
#     # return NA if -infinity
#     if (is.infinite(res1$ParamSummary$LogLik) | (res1$ParamSummary$LogLik <= -1e5)){
#       return(rep(NA, 8))
#     }
#     
#     # if not, continue
#     # extract initial ML parameters
#     init_ML <- c(c(res1$ParamsInModel$Sxx)[c(1,2,4)],
#                  c(res1$ParamsInModel$vX0),rep(0.1,3))
#     
#     # set lower and upper bound of optimization
#     lb <- c(0, -100, 0, -100, -100, 0, -100, 0)
#     ub <- c(100, 100, 100, 100, 100, 100, 100, 100)
#     
#     # run optim
#     opt_res <- optim(par = init_ML, fn = fopt_mvs, method = "L-BFGS-B",
#                      lower = lb, upper = ub, control = list(fnscale = -1),
#                      ntips = ntips, ids = ids)
#     
...
# Xsim <- Xsim_4[[3]]
# 
# test_mvs <- ml_me(tree, Xsim, method = "mvSLOUCH")
